...
func main() {
	fmt.Println("Running SDWAN link selection demo.")

	// Set random seed for reproducibility
	rd := rand.New(rand.NewSource(42))

	// Define the problem parameters
	numGroups := 8   // number of prefix groups to fit links into
	numLinks := 200  // number of links, each associated with a single device
	numDevices := 50 // number of devices

	// Generate random capacities for links (in Mbps)
	capacities := make([]float64, numLinks)
	for i := range capacities {
		capacities[i] = float64(rd.Intn(801)+200) + rd.Float64()
	}
	...
	solver.SetOptimizationObjective(objective, mip.Minimize)
	...
	// Note that the solver will return the best solution found within the time limit
	// Without optimality guarantees
	foundOptimal, err := solver.Solve(5 * time.Second)
	if err != nil {
		panic(err)
	}
	...
	fmt.Printf("Max Device Usage fraction: %f\n", globalUsageUB.Value())
